...
Five [modules](https://predictiveecology.github.io/spadesCBM/lexicon.html#modules) need to be run in tandem for a [spadesCBM](https://github.com/PredictiveEcology/spadesCBM.git) simulation. 
The first module, [CBM_defaults](https://github.com/PredictiveEcology/CBM_defaults.git), reads in defaults CBM parameters for Canada. The next two modules, [CBM_dataPrep_SK](https://github.com/PredictiveEcology/CBM_dataPrep_SK.git) and [CBM_dataPrep](https://github.com/PredictiveEcology/CBM_dataPrep.git), are data preparation SpaDES modules, where input data and spatial layers are assembled and prepared for a specific study area. The [CBM_dataPrep_SK](https://github.com/PredictiveEcology/CBM_dataPrep_SK.git) module is study-area and scenario specific. 
...
The [events](https://predictiveecology.github.io/spadesCBM/lexicon.html#events) in the[CBM_defaults](https://github.com/PredictiveEcology/CBM_defaults.git), [CBM_vol2biomass](https://github.com/PredictiveEcology/CBM_vol2biomass.git), and [CBM_dataPrep_SK](https://github.com/PredictiveEcology/CBM_dataPrep_SK.git) modules need to be run only once (note that they will be [cached](https://predictiveecology.org/training/_book/Caching.html) in our example). [CBM_dataPrep](https://github.com/PredictiveEcology/CBM_dataPrep.git) has four data processing events, but also has a fifth event where disturbances are processed for each simulation year. There are two options for this disturbance-related event. If the user supplies their own disturbances, either directly or through the link of a disturbance model, these are read in with the `readDisturbances` event. If not, the module will use the default  `readDisturbancesNTEMS` event and use national disturbance information from NTEMS for the study area. 

These four modules provide the inputs to the [CBM_core](https://github.com/PredictiveEcology/CBM_core.git) module, where carbon-transfers between pools are applied on a specified time step (in our example, yearly). 
